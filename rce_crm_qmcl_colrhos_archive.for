...
c Aug 6: saving this version of this so I can create a new version 
c Which uses Fortran 90 syntax to speed up code 
c A little bit is already in this one, in thermo_qmcl
...
c     phi is eigenfunctions matrix 
      real rho(L,L,nx)
      real U(L,L), rho_interm(L,L), trans_U(L,L)
...
c      do kk=1,L 
c      do ll=1,L
c            rho_interm(kk,ll) = rho(kk,ll,j)
c      enddo
c      enddo

      rho_interm = rho(1:L, 1:L, j)
     ...
     &  N_qm, N_real, D, nx, nz, j, trans_phi)
...
c      do ll=1,L
c      do kk=1,L 
c            S_qv(kk,ll) = S_array_qv(kk,ll,1,i)
c            S_qr(kk,ll) = S_array_qv(kk,ll,2,i)
c            S_qc(kk,ll) = S_array_qv(kk,ll,3,i)
c            S_th(kk,ll) = S_array_qv(kk,ll,4,i)
c      enddo
c      enddo
      ...
      enddo
      print*, "Pre-evolve_rho"
      
      CALL evolve_rho_fast(rho_interm, U, L, trans_U)
...
c      print*, "Covariate updated"
     ...
     .+ (phi(kk,ii)*Y(kk,1))*phi(kk,jj)
      ...
      subroutine evolve_rho_fast(rho,U, spectral_res, trans_U) 
            integerspectral_res
...
cc    Edit above dimensions when changing what variables/data used to train 
      ...
      do i=1,D
            do j=1,nz
c DF: new_x has the D variables for a given z point, followed by the D vars for the next point, etc
                  new_x((D*(j-1)) + i,1) = 
     &new_x_array(i,j)
            enddo
      enddo
      ...
      x_matrix((D*(k-1))+j,i) = 
     & training_data((nx*(k-1))+x_cord, i,j)
