...
c     Same as the coarsekoop singlecol inisitalS nokoop but
c     uses qv, theta, ux, and uz as the forcings
...
cc    Was 20 in first run
      parameter(ntime0=10000) ! 24/(4*60) hrs
...
cc thermodynamics
      ...
      dimension qv_storage(1,nz), qc_storage(1,nz)
      dimension th_storage(1,nz), qr_storage(1,nz)
      dimension ux_storage(1,nz), uz_storage(1,nz)
...
cc constants
      ...
      parameter(L = 1000, ntime_training=10000, N_qm=10000) 

c      parameter(N_real = 500.0)
      integer D
      integer update_index
      integer evolve_index
      real N_real
c      integer res_var

c     double precision training_vec(nxz*N)
      real training_vec(51*10000*4)
      real T_bar_vec(nz*6)
      real T_bar(nz,6)
      real sigma_T(6)
c     ^^ 7 = number of distinct variables in training set
      real phi_vec(10000*1000)
      real S_vec(51*1000*1000*4)
      integer i,ii,j,jj
...
c     DF :Fixed for multi-var
      dimension U(L,L), trans_U(L,L), S_save(L,L)
c      dimension phi(number_training_pts,L)
      dimension phi(10000,L)
      dimension trans_phi(L,10000)

      dimension training_data_qm(nz,N_qm, 4)
c      dimension training_data_formatted(nx,nz,N_qm,7)
c      dimension training_data_formatted_b(nx,7*nz,N_qm)
...
cc    PCA stuff: 
      integer pca_res
      parameter(pca_res = 25)
c      dimension training_pca(nx,pca_res, N_qm)
      ...
      integer nys_ind
      real nys_ind_real 
      real nys_int_len_real
      integer nys_int_len
      real training_pca_nys(pca_res, 10000)
      real trans_phi_nys(1000, 10000)
      real phi_nys(10000, 1000)
      real training_resh(nz*4,N_qm)
c      real training_pca_nys(nx, pca_res, nys_int_len)
c      real trans_phi_nys(L, nys_int_len)
c      real phi_nys(nys_int_len, L)

      evolve_index = 1
      nys_ind = 1
      nys_int_len = 10000
      nys_ind_real = 1.0 
c      nys_int_len_real = N_real/nys_ind_real 
      nys_int_len_real = 10000.0
      ...
      test_vec_2 = matmul(test_mat, test_vec)


      training_pca_nys = 0.0
cc    End PCA stuff
...
c          close(10)
      ...
      open(59,FILE="S_1000L_wind_200eps_10kN.bin",
     . FORM='UNFORMATTED',
     . ACTION='READ', STATUS='OLD', ACCESS='STREAM')
      ...
      do jj=1,nz
          do i=1,L
              do j=1,L
                  S_matrix(i,j,ii,jj) = S_vec(
     &i + (j-1)*L + (jj-1)*L*L + (ii-1)*L*L*nz)                  
              end do
          end do
      enddo
      enddo
      ...
      open(59,
     . FILE="eigenfunctions_1000L_wind_200eps_10kN.bin"
     . ,FORM='UNFORMATTED',
     . ACTION='READ', STATUS='OLD', ACCESS='STREAM')
      read(59) phi_vec
      close(59)
      ...
      open(77,FILE="T_bar_wind_10kN.bin",
     .FORM='UNFORMATTED',
     . ACTION='READ', STATUS='OLD', ACCESS='STREAM')
      read(77) T_bar_vec
      close(77)
      ...
      open(79,FILE="sigma_T_wind_10kN.bin",
     .FORM='UNFORMATTED',
     . ACTION='READ', STATUS='OLD', ACCESS='STREAM')
      read(79) sigma_T
      close(79)
      ...
      open(49,
     . FILE="training_1000L_wind_200eps_10kN.bin"
     .,FORM='UNFORMATTED',
     . ACTION='READ', STATUS='OLD', ACCESS='STREAM')
...
c           open(unit=12, file = training_text_file, status = 'old')
      do ii=1,pca_res
      do i=1,nz*4
c            pca_coeff_mat_trans(ii,i) = pca_coeff_vec(
c     .(ii-1)*nz*4 + i)
     ...
     .(i-1)*pca_res + ii)
      ...
      open(57,FILE="pca_mu.bin",FORM='UNFORMATTED',
     . ACTION='READ', STATUS='OLD', ACCESS='STREAM')
c     training_vec_2 seems to be the working, updated course data
      read(57) pca_mu
      close(57)
...
c     DF: training data contains resolved and unresolved vars in 1st dim 
     ...
     .(ii-1)*nz*N_qm + j +(i-1)*(nz))
      ...
      do i=1,nz
            do j=1,N_qm
                  do ii=1,4
                  training_resh(i+(ii-1)*nz,j) = training_data_qm(
     &i,j,ii)
                  enddo
            enddo
      enddo
c      training_data_qm = reshape(training_vec, shape(training_data_qm))
...
c     Note: training_data_qm should be the SHIFTED training data (i.e. T_tilde) 
      CALL generate_qmcl_objects_qv(
     & L, N_qm, nx, nz, 
     & phi, ntime_training, nx*nz, N_real, N_qm, U,
     & trans_U, pca_coeff_mat_trans, pca_res, training_pca_nys, pca_mu,
     &T_bar, sigma_T, nys_ind, nys_int_len, phi_nys, trans_phi_nys)

c      S_save = S_matrix(1:L,1:L,1,2)

c       open(113,file=
c     .'/scratch/dfreeman/EULAG Fortran'//
c     .'/VSCode Workspace'//
c     .'/EULAG/QM Output Data/'//'S_qv',
...
cc open tape to read from:
      if(irstrt.eq.1)
     . open(31,file='walker_bnch_2.d1-10',
...
c       call plot_2(theta,qv,qc,qr,nx,nz,xx,zz,scr1,th_e,tm_e)
      ...
      ifl=20   ! number of files to read from tape
...
c     Still need to add code to read in training data and phi matrices
      ...
      qv_storage(1,:) = qv(1500,:)
      qc_storage(1,:) = qc(1500,:)
      qr_storage(1,:) = qr(1500,:)
      th_storage(1,:) = theta(1500,:)
      ux_storage(1,:) = ux(1500,:)
      uz_storage(1,:) = uz(1500,:)


      call thermo_qmcl_qv(qv_storage, qc_storage, qr_storage,
     & th_storage, ux_storage, uz_storage, L,
     & S_matrix, 0.0, 
     & phi, U, N_qm, 1.0, N_real, D, nx, nz, rho,
     & T_bar, sigma_T, update_index, trans_U, 
     & trans_phi_nys, 0.0, pca_res,
     & training_pca_nys, pca_coeff_mat_trans, nys_ind, nys_int_len,
     & nys_int_len_real, phi_nys, pca_mu, training_resh,
     & evolve_index)
      ...
      qv(1500,:) = qv_storage(1,:)
      qc(1500,:) = qc_storage(1,:)
      qr(1500,:) = qr_storage(1,:)
      theta(1500,:) = th_storage(1,:)

      endif
       

      call thermo(theta,qv,qc,qr,
     &ft,fqv,fqc,fqr)
...
c        if(itime/ntape*ntape.eq.itime) then
c      call tape_wr(ux,uz,uxp,uzp,theta,qv,qc,qr,
c     .             fx,fz,ft,fqv,fqc,fqr,p,nx,nz)
cc       endif
...
c     .'/scratch/dfreeman/EULAG Fortran'//
c     .'/VSCode Workspace'//
c     .'/EULAG/QM Multop d10/'//fileName,
c     . form='unformatted',status='new')
c         write(39) S_matrix
c       
c        close(39)
c      endif
     ...
     .'/VSCode Workspace'//
     .'/EULAG/QM Forced Data d10 Coupled/'//fileName,
...
cc    DF: QMCL Subroutines 
      ...
      subroutine generate_qmcl_objects_qv(
     & L, number_training_pts, nx, nz, 
     & phi, ntime_training, D, N_real, N_qm, U,
     & trans_U, pca_coeff_matx_t, pca_dims, training_pca_nys, pca_mu,
     & T_bar, sigma_T, nys_ind, nys_int_len, phi_nys, trans_phi_nys)
...
c currently existing variables
      ...
      integer nys_ind, nys_int_len
      real mu_spread_nys(nz*4, nys_int_len)
      real phi_nys(nys_int_len, L)
      real phi_1_nys(nys_int_len-1, L)
      real phi_2_nys(nys_int_len-1, L)
      real pca_data_interm_nys(nz*4, nys_int_len)
      real training_pca_nys(pca_dims, nys_int_len)
      real trans_phi_nys(L, nys_int_len)
...
c     CHANGE 3rd INDEX WITH DIFFERENT DATASET
      real tilde_data_trunc(nz*4, ntime_training)
      real pca_data_interm(nz*4, ntime_training)
      real pca_coeff_matx_t(pca_dims, nz*4)
      real pca_mu(nz*4)
      real pca_mu_spread(nz*4, number_training_pts)
...
c      real training_pca(nx,pca_dims, ntime_training)
      real T_bar(nz,6)
      real sigma_T(6)
      real pca_intermediate
      phi_nys = 0.0
      trans_phi_nys = 0.0;
      phi_data_interm_nys = 0.0;
...
c      pca_mu_spread = spread(pca_mu(:), 2,number_training_pts)

c      do j=1,nx
c      pca_data_interm = tilde_data_trunc(j,:,:)
c      pca_data_interm = pca_data_interm - pca_mu_spread
c      training_pca(j,:,:) = matmul(pca_coeff_matx_t, pca_data_interm)
c      enddo

cc Commented out part::

      

      do j=1,L
      do m=1,nys_int_len
      phi_nys(m,j) = phi(m*nys_ind, j)
      trans_phi_nys(j,m) = phi_nys(m,j)
      enddo
      enddo
ccc END
...
c      U = (1/num_tr_pts_real)*matmul(tr_phi_1, phi_2)
     ...
     & theta, ux, uz, L, 
     & S_array_qv, random, phi, U, N_qm
     &, measurement_epsilon, N_real, D, nx, nz,
     & rho, T_bar_therm, sigma_T, update_index, trans_U,
     & trans_phi_nys, bullshit_c, pca_terms,
     & pca_data, pca_coeff_mat_tpose,
     & nys_ind, nys_int_len, nys_int_len_real,
     & phi_nys, pca_mu, training_data, evolve_index)
c     phi is eigenfunctions matrix 
      ...
      integer nys_int_len 
      real nys_int_len_real
      real phi_nys(10000,L)
      real rho(L,nx)
      real U(L,L), trans_U(L,L)
      real rho_interm(L)
      real vec_interm(L)
      real S_array_qv(L,L,4,nz)
      real S_qv(L,L),S_qr(L,L),S_qc(L,L),S_th(L,L)
      real training_data(nz*4,N_qm)
c      real training_data(nx*nz, ntime_training, 7)
      real random
      real phi(ntime_training, L)
      real trans_phi_nys(L, 10000)
      ...
      integer update_index
      real trace_val 
      real qv(1,nz), qc(1,nz), qr(1,nz)
      real ux(1,nz), uz(1,nz)
      real theta(1,nz)
      real N_real
      real T_bar_therm(nz,6)
      real sigma_T(6)
      real sigma_unres(6,1)
c      real T_bar_unres(4,1)
c      real interm_prime(4,4)
      real closure_terms(4,1)
c      real covariate_array(3,3,3)
c     ^length and width of local nbhd, plus # of resolved vars (3) 
      integer D 
      integer pca_terms
      real covariate_array(nz,4)
      real qv_interm, qr_interm, qc_interm
      real th_interm
      real qv_placehold(L)
      real qr_placehold(L)
      real qc_placehold(L), th_placehold(L)
      
      real qv_interm_prime, qr_interm_prime
      real qc_interm_prime, th_interm_prime
      real diag1(L,1),diag2(L,1),diag3(L,1),diag4(L,1)
c      real training_data_reshape(nx,D*nz,N_qm)
      real bullshit_c
      real pca_data(pca_terms, nys_int_len)
      real pca_coeff_mat_tpose(pca_terms, 4*nz)
      real qv_hold,qr_hold,qc_hold
      real intermediate_for_error
      do ll=1,6
            sigma_unres(ll,1) = sigma_T(ll)
      enddo 
...
cc     Evolve rho by 10 step koopman

      vec_interm = matmul(trans_U, rho_interm)
      trace_value= 0.0
      do kk=1,L
            trace_value = trace_value + 
     &(vec_interm(kk))**2
      enddo 
      rho_interm = (1.0/sqrt(trace_value))*vec_interm
      


      do gg=1,nz
            intermediate_for_error = 0.0
            intermediate_for_error = (qv(j,gg) - 
     &T_bar_therm(gg,1))/sigma_unres(1,1)
            covariate_array(gg,1)=intermediate_for_error
            intermediate_for_error =(theta(j,gg) - 
     &T_bar_therm(gg,4))/sigma_unres(4,1)
            covariate_array(gg,2)=intermediate_for_error
            intermediate_for_error=(ux(j,gg) - 
     &T_bar_therm(gg,5))/sigma_unres(5,1)
            covariate_array(gg,3) = intermediate_for_error
            intermediate_for_error=(uz(j,gg) - 
     &T_bar_therm(gg,6))/sigma_unres(6,1)
            covariate_array(gg,4) = intermediate_for_error

c            covariate_array(5,kk)=(ux(j,kk) - T_bar(kk,5))/sigma_T(5)
c            covariate_array(6,kk)=(uz(j,kk) - T_bar(kk,6))/sigma_T(6)
      enddo
      gg = 1

      CALL update_rho_pca(rho_interm,phi_nys,covariate_array,trace_val
     &  , measurement_epsilon,L,
     &  10000, 10000.0, 4, nx, nz, j, trans_phi_nys, tr_value,
     & pca_terms, training_data, pca_coeff_mat_tpose, pca_mu)
     

c     Update fluxes based on quantum state
      ...
      qv_placehold = matmul(S_qv, rho_interm)
      qr_placehold = matmul(S_qr, rho_interm)
      qc_placehold = matmul(S_qc, rho_interm)
      th_placehold = matmul(S_th, rho_interm)

      do k=1,L  
      qv_interm = qv_interm + qv_placehold(k)*rho_interm(k)
      qc_interm = qc_interm + qc_placehold(k)*rho_interm(k)
      qr_interm = qr_interm + qr_placehold(k)*rho_interm(k)
      th_interm = th_interm + th_placehold(k)*rho_interm(k)
      enddo
      ...
      qv(j,i) = (qv_interm*sigma_unres(1,1))+T_bar_therm(i,1)
      qr(j,i) = (qr_interm*sigma_unres(2,1))+T_bar_therm(i,2)
      qc(j,i) = (qc_interm*sigma_unres(3,1))+T_bar_therm(i,3)
      theta(j,i) = (th_interm*sigma_unres(4,1))+T_bar_therm(i,4)
...
!$OMP END PARALLEL DO
     ...
     .+ (phi(kk,ii)*Y(kk,1))*phi(kk,jj)
      ...
      subroutine update_rho_pca(rho, phi_nys, new_x_array,bullshit,
     & epsilon, spectral_res, training_len, N_real, D_up, nx, nz, x_cord
     &, trans_phi_nys, bullshit_b, pca_terms, training, C_T, mu_pca)
...
cc    D = number of resolved variable dimensions 
      ...
      real phi_nys(training_len, spectral_res)
      real trans_phi_nys(spectral_res, training_len)
...
c      real x_matrix_new(D_up*nz, training_len)
c      real x_matrix_newnew(D*nz, training_len)
      real new_x_array(nz, D_up)
...
c      real training_data_reshape(nx,D*nz,training_len)
      real bullshit_b
c      real training_interm(nz,training_len,D_up)
      
      real F_reshape(training_len, spectral_res)
      real diag1(spectral_res)

      integer pca_terms
      real training(nz*4, training_len)
      real C_T(pca_terms, D_up*nz)
      real new_x_pca(pca_terms)
      real new_x_spread(nz*4,training_len)
      real x_matrix(nz*4, training_len)
...
c      do k=1,pca_terms
c            do l=1,D_up*nz
c                  new_x_pca(k) = new_x_pca(k) + C_T(k,l)*new_x(l)
cc                  print*, new_x_pca(k)
c            enddo
c      enddo

c      new_x_pca = matmul(C_T, new_x)

c      call dgemv('N',pca_terms,D_up*nz,alpha_2,C_T, pca_terms
c     &,new_x,1,0.0,new_x_pca,1)

c      x_matrix = training_data_reshape(x_cord,:,:)
      new_x_spread = spread(new_x(:), 2,training_len)
      x_matrix = (training-new_x_spread)**2
c      x_matrix_newnew = x_matrix_new**2
c      column_sums = SUM(x_matrix_newnew, dim=1)
      column_sums = SUM(x_matrix, dim=1)

      F = EXP(column_sums*(-1.0/1000.0))
      F_reshape = spread(F, 2, spectral_res)

      phi_prime = phi_nys*F_reshape

      G = (1/N_real)*matmul(trans_phi_nys, phi_prime)
      new_rho = matmul(G,rho)

      tr_value = 0.0

      tr_value = SUM(new_rho**2) 
