...
cc  nember of time steps:
      parameter(ntime0=2500) ! 10*24 hrs
...
cc grid:
      ...
      irstrt=0     ! 0-start from initial data, 1-start from tape

cc open tape to write to:
       open(21,file='walker_bnch.d1-10',
...
cc  write data to tape:
     ...
     .             fx,fz,ft,fqv,fqc,fqr,p,nx,nz)
      ...
      baseFileName = "qv_output"

      write(fileName, '(A,I4,A)') baseFileName, itime, ".d1-10"

       open(93,file=
     .'/Users/dfreem/Desktop/VSCode Workspace'//
     .'/EULAG/Saved Data/'//fileName,
     . form='unformatted',status='new')
         write(93) qv
       
        close(93)

        baseFileName = "qr_output"
...
cc This wouldn't involve worrying about cosening 
cc
...
cc surface data:
      ...
      e=-cp/rg
       ...
       thsrf(i)=th_e(1) + 1.  
c       thsrf(i)=th_e(1) + 1. + 
c     *     2.0*cos(2.*pi*float(i-1)/float(nx-1) - pi)
