...
c  7.15.24 Update
cc Fixed error with typo ('rh' instead of 'th' in thermo_qmcl)
cc Changed 'th' to 'thermo'
cc Changed order of operations in thermo+qmcl for updating

c  7.23 update: Tried speeding up by inputting transpose of training
c  data into thermo_qmcl and looping over inner rather than outer index
...
cc    Was 20 in first run
      parameter(ntime0=50) ! 24/(4*60) hrs
...
c           open(unit=12, file = training_text_file, status = 'old')
      print*, "Training vec outputs"
      print*, training_vec(1)
      print*, training_vec(25)
      print*, training_vec(714178500)
...
c            training_data_qm(nx*nz+j,i,ii) = 
c     .training_vec((ii-1)*nx*nz*N_qm + (nx*nz*N_qm) 
c     .+j+(i-1)*(nx*nz))
            end do
c      print*, i
      end do
...
c           enddo      ! TIME LOOP
c          open(91, file = 'qv_text',status='new')
c           write(91, *) qv_total
c           close(91)
...
c      open(92,file='qv_output.d1-10',
c     . form='unformatted',status='new')
c       write(92) qv_total
c       
        stop
...
c     ^length and width of local nbhd, plus # of resolved vars (3) 
      integer D 
      real covariate_array(7)
      real qv_interm(L,1), qr_interm(L,1), qc_interm(L,1)
      real th_interm(L,1)
      ...
      do ll=1,L
      do kk=1,L 
            S_qv(kk,ll) = S_array_qv(kk,ll,1,i)
            S_qr(kk,ll) = S_array_qv(kk,ll,2,i)
            S_qc(kk,ll) = S_array_qv(kk,ll,3,i)
            S_th(kk,ll) = S_array_qv(kk,ll,4,i)
      enddo
      enddo
C$OMP PARALLEL DO
      do j=1,nx
...
c      covariate_array(kk,ll,1) = ux(j-2+kk, i-2+ll)
c      covariate_array(kk,ll,2) = uz(j-2+kk, i-2+ll)
c      covariate_array(kk,ll,3) = p(j-2+kk, i-2+ll)

c      enddo
c      enddo

      covariate_array(1)=(qv(j,i) - T_bar(i,1))/sigma_T(1)
      covariate_array(2)=(qr(j,i) - T_bar(i,2))/sigma_T(2)
      covariate_array(3)=(qc(j,i) - T_bar(i,3))/sigma_T(3)
      covariate_array(4)=(theta(j,i) - T_bar(i,4))/sigma_T(4)
      covariate_array(5)=(ux(j,i) - T_bar(i,5))/sigma_T(5)
      covariate_array(6)=(uz(j,i) - T_bar(i,6))/sigma_T(6)
      covariate_array(7)=(p(j,i) - T_bar(i,7))/sigma_T(7)

c     Evolve rho 
      vec_interm = matmul(U,rho_interm)
      trace_value= 0
      do kk=1,L
     ...
     .(vec_interm(kk,1))**2
      ...
      CALL update_rho(rho_interm,phi,covariate_array,
     &  training_data, measurement_epsilon,L,
     &  N_qm, N_real, D, nx, nz, j, i)

c      print*, "Update rho finished"

            
      do kk=1,L 
      do ll=1,L
            rho(kk,j,i) = rho_interm(kk,1)
      enddo
      enddo

cc    Vectorized version of tr(rho A)

      qv_interm = matmul(S_qv, rho_interm) 
      qr_interm = matmul(S_qr, rho_interm)
      qc_interm = matmul(S_qc, rho_interm)
      th_interm = matmul(S_th, rho_interm)
      ...
      do kk=1,L
      qv_interm_prime =  qv_interm_prime + 
     .(qv_interm(kk,1)*rho_interm(kk,1))
      qr_interm_prime = qv_interm_prime + 
     .(qr_interm(kk,1)*rho_interm(kk,1))
      qc_interm_prime = qv_interm_prime + 
     .(qc_interm(kk,1)*rho_interm(kk,1))
      th_interm_prime = qv_interm_prime + 
     .(th_interm(kk,1)*rho_interm(kk,1))
      enddo

cc    
      qv(j,i) = (qv_interm_prime*sigma_T(1))+T_bar(i,1)
      qr(j,i) = (qr_interm_prime*sigma_T(2))+T_bar(i,2)
      qc(j,i) = (qc_interm_prime*sigma_T(3))+T_bar(i,3)
      theta(j,i) = (th_interm_prime*sigma_T(4))+T_bar(i,4)

c                 Should test to make sure this works...might want to add
c                 an additional dummy variable 

      enddo
c      print*, rho(1,1,1,1)
c      print*, "Moving on to nx="
c      print*, j
...
c      print*, "Covariate updated"
     ...
     .+ (phi(kk,ii)*Y(kk,1))*phi(kk,jj)
      ...
      subroutine evolve_rho_fast(rho,U, spectral_res) 
            integerspectral_res
            real U(spectral_res, spectral_res)
            real trans_U(spectral_res, spectral_res)
            real dummy(spectral_res, spectral_res)
            real rho(spectral_res, spectral_res)
            ...
            do i=1,50
                  ...
                  enddo
            enddo
            dummy = matmul(trans_U, rho)
            intermediate = matmul(dummy,U)
            tr_val = 0.0
            CALL trace(tr_val, intermediate, spectral_res)
            print*, "Trace Value" 
            print*, tr_val
            rho = (1/tr_val)*intermediate
            
c            print*, "Ran evolve_rho: printing rho(1,1)" 
c            print*, rho(1,1)
      end subroutine evolve_rho_fast
                     
      subroutine update_rho(rho, phi, new_x_array,training_data,
...
cc    D = number of resolved variable dimensions 
      ...
      real N_real
      real rho(spectral_res, 1)
      real new_rho(spectral_res, 1)
...
cc    Edit above dimensions when changing what variables/data used to train 
      ...
      new_x(1,1) = new_x_array(1)
      new_x(2,1) = new_x_array(2)
      new_x(3,1) = new_x_array(3)
      new_x(4,1) = new_x_array(4)
      new_x(5,1) = new_x_array(5)
      new_x(6,1) = new_x_array(6)
      new_x(7,1) = new_x_array(7)

      do i=1,training_len
      do j=1,D
      x_matrix(j,i) = training_data((nx*(z_cord-1))+x_cord, i,j)
cc    First var picks the spatial point to be the x and z coord we chose 
      end do
      end do
c      print*, "x_matrix generated"
      do j=1,training_len
      dummy = 0
            do k=1,D 
            dummy = dummy + (x_matrix(k,j) - new_x(k,1))**2
            end do
      F(j) = EXP(dummy*(-1))  
      end do
c      print*, "F generated"
      ...
      G = (1/N_real)*matmul(tr_phi,phi_prime)
c     DF - try comment section
      new_rho = matmul(G, rho)
      tr_value = 0.0
      do i=1,spectral_res
      tr_value = tr_value + (new_rho(i,1))**2
      enddo
