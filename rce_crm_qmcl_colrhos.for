...
c     Still need to add code to read in training data and phi matrices
     ...
     & trans_phi, training_data_formatted)
       endif
...
c      print*, "Finished generate S"
     ...
     & trans_phi, training_data_reshape)
...
c     ^length and width of local nbhd, plus # of resolved vars (3) 
      ...
      real training_data_reshape(nx,nz,N_qm,D)
...
c      print*, "Covariate updated"
     ...
     .+ (phi(kk,ii)*Y(kk,1))*phi(kk,jj)
      ...
      subroutine evolve_rho_fast(rho_temp,U, spectral_res, trans_U) 
            ...
            real rho_temp(spectral_res, spectral_res)
...
ccccc          
      ...
      real x_matrix_newnew(D*nz, training_len)
...
cc    Edit above dimensions when changing what variables/data used to train 
      ...
      real training_data_reshape(nx,nz,training_len,D)
...
c      enddo
      ...
      training_interm = training_data_reshape(x_cord,:,:,:)
c      x_matrix = reshape(training_data_reshape(x_cord,:,:,:), shape(x_matrix))
      x_matrix = reshape(training_interm, shape(x_matrix))
      new_x_spread = spread(new_x(:,1), 2,training_len)
c      x_matrix = (x_matrix-spread(new_x(:,1), 2, training_len))**2
      x_matrix_new = x_matrix-new_x_spread
      x_matrix_newnew = x_matrix_new**2
      column_sums = SUM(x_matrix_newnew, dim=1)

    
      F = EXP(column_sums*(-1.0/100.0))
c      F_reshape = reshape(F, shape(phi))
      F_reshape = spread(F, 2, spectral_res)

      phi_prime = phi*F_reshape

c      do l=1,spectral_res
c      do k=1,training_len
c      phi_prime(k,l) = phi(k,l)*F(k)
c      end do          
c      end do

c      new_x_spread = spread(new_x, 2, training_len)
c      x_matrix = (x_matrix - new_x_spread)**2
c      x_matrix = x_matrix**2
c      column_sums = SUM((x_matrix - new_x_spread)**2, dim=1)

c     Fully vectorized version
c      F = EXP(column_sums*(-1.0/200.0))
c      F_spread = spread(F, 2, spectral_res)
c      phi_prime = phi*F_spread
cccccccccccccccccc
      

c      do j=1,training_len
c            F(j) = EXP(column_sums(j)*(-1.0/50.0))
c      enddo
...
c            do k=1,D*nz 
c            dummy = dummy + (x_matrix(k,j) - new_x(k,1))**2
c            end do

c      F(j) = EXP(dummy*(-1.0/50.0))  
c      end do 
c      print*, "F generated"
