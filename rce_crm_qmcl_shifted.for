...
C$OMP PARALLEL DO
      ...
      do kk=1,L 
      do ll=1,L
            rho_interm(kk,ll) = rho(kk,ll,j,i)
      enddo
      enddo
...
c      enddo
      ...
      CALL evolve_rho_fast(rho_interm, U, L)
...
c      print*, "Update rho finished"
      ...
      do kk=1,L 
      do ll=1,L
            rho(kk,ll,j,i) = rho_interm(kk,ll)
      enddo
      enddo

      qv_interm = matmul(rho_interm,S_qv) 
      qr_interm = matmul(rho_interm,S_qr)
      qc_interm = matmul(rho_interm,S_qc)
      th_interm = matmul(rho_interm,S_th)

      CALL trace(qv_interm_prime, qv_interm, L)
      CALL trace(qr_interm_prime, qr_interm, L)
      CALL trace(qc_interm_prime, qc_interm, L)
      CALL trace(th_interm_prime, th_interm, L)
     ...
     .+ (phi(kk,ii)*Y(kk,1))*phi(kk,jj)
      ...
      subroutine evolve_rho_fast(rho,U, spectral_res) 
            ...
            do i=1,50
                  ...
                  enddo
            enddo
            standin = matmul(trans_U, rho)
            intermediate = matmul(standin,U)
            tr_val = 0.0
...
cc    D = number of resolved variable dimensions 
      ...
      real N_real
      real rho(spectral_res, spectral_res)
      real new_rho(spectral_res, spectral_res)
...
c      interm_G = matmul(rho, G)
      new_rho = matmul(G, matmul(rho, G))
c      new_rho = matmul(G,matmul(rho,G))
      tr_value = 0.0
      CALL trace(tr_value, new_rho, spectral_res) 
      rho = new_rho/tr_value
